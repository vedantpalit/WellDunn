...
# Run python Llama_experiment.py
...
import shutil, sys
import torch
access_token_read = "" #enter your hf read token
...
tokenizer=AutoTokenizer.from_pretrained("pankajmathur/orca_mini_3b", output_attentions=True)
...
class llamaClass(torch.nn.Module):
    def __init__(self):
        ...
        # self.tokenizer = AutoTokenizer.from_pretrained("pankajmathur/orca_mini_3b")
        # self.model = AutoModel.from_pretrained("pankajmathur/orca_mini_3b", return_dict=True,load_in_4bit=True,use_cache=False)
        self.tokenizer = AutoTokenizer.from_pretrained("pankajmathur/orca_mini_3b")
        self.model = AutoModel.from_pretrained("pankajmathur/orca_mini_3b", return_dict=True,output_attentions=True)
        self.dropout = torch.nn.Dropout(0.05)
        self.linear = torch.nn.Linear(3200, dimension)
...
from sklearn.metrics import classification_report #get the classification report
